...
| 3 ✅ | 差し戻し＋コメント→再生成ループ | **コード完成**。承認ページに差し戻しボタン+コメント→repository_dispatchで再生成。要PAT設定 |
| 4 ✅ | 公開cron（承認→当日公開） | **コード完成・実投稿成功**（2026-06-05をライブ公開済み） |
| 5 ✅ | フィード投稿の統合 | **コード完成**。カルーセルも生成→承認→公開を同フロー（CI に Python+Playwright 追加） |
