...
- 数値だけでなく、制度の仕組み・因果関係・適用条件も正確であること。
  「AすればBになる」という因果関係を書く場合、その因果が税法上正しいか必ず確認する
- 一般論（どの月・どの条件でも成り立つ話）を特定条件の固有メリットとして語らない
  例: ×「○月決算にすれば免税期間が延びる」（免税期間は設立日との間隔で決まり、決算月固有の効果ではない）

【参考資料と依頼テーマが矛盾する場合の扱い（最重要）】
依頼のテーマ・切り口が、ある効果や有利さを前提として書かれていることがある。
しかしその前提自体が税法上誤っている場合、前提に従ってはいけない。
- 【税務参考資料】の記載が依頼テーマの前提と矛盾する場合は、**必ず参考資料を優先**する
- 前提が成り立たないとわかったら、その前提に沿った台本を書かず、
  「実際にはこうである」という正しい内容に切り替えて構成する
- 「テーマがそう書いてあるから」という理由で、誤った因果関係を説明してはいけない
例: テーマが「年払いに変えると控除が増える」でも、資料に「控除額はその年に支払った
    金額の全額」とあるなら、支払方法で控除額は変わらない。増えるかのように書かない。
...
【使用禁止の専門用語・カタカナ用語（一般視聴者に伝わりにくいため）】
× 「クレジットノート」 → ○ 「返品の書類」「返金の伝票」「返還請求書」など、状況に応じて平易な日本語で言い換える
※ 一般的に日本の商取引で馴染みがない英語カタカナ用語は使わない。専門用語を出すなら必ず「＝◯◯のこと」と平易な補足を添えるか、そもそも別の言い方に置き換える。
...
export async function generateScript(
  ...
  if (isEdit) {
    ...
    userContent =
      ...
      `【最優先の例外ルール：読み方・発音の指示】\n` +
      `修正指示が「読み方」「読み」「発音」「〜と読ませて」「(かっこ内のかな)で読んで」など、` +
      `音声の読み上げ方に関するものだった場合、台本テキストでは対応できません` +
      `（読みは音声合成側の辞書 voiceText.js / voicevoxDict.js で処理されます）。\n` +
      `その場合は台本を一切変更せず、渡された JSON をそのまま出力してください。\n` +
      `特に以下は絶対にやってはいけません:\n` +
      `- 表記をひらがな・カタカナに書き換える（例: BtoC → びーとぅーしー）\n` +
      `- 読みにくい語を別の語や日本語訳に置き換える（例: BtoC → 企業から個人へ）\n` +
      `- 該当語を削除する、言い回しごと差し替える\n` +
      `表示（字幕）は元の表記のままが正しく、読みだけを別の仕組みで直します。\n\n` +
