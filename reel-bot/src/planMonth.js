...
async function planTopics(monthStr, slots, personas, pastTopics) {
  ...
  const system =
    ...
    "【税務正確性の絶対ルール（厳守）】\n" +
    "topic と angle は必ず日本の税法・実務と整合していること。特に因果関係・時期・金額の記述は正確に。以下の典型的な誤りを絶対に含めない:\n" +
    "- ×「今年○月に稼ぐと△月に税金が増える」= 予定納税は前年基準なので、今年の売上は今年の予定納税に影響しない\n" +
    "- ×「予定納税」と「中間申告」を混同する = 前者は個人所得税（納期7月末・11月末）、後者は消費税・法人税で仕組みが別\n" +
    "- ×「収入○万円超で申告必要」= 判定は所得（収入−経費）で行う。給与所得者副業は20万円超、専業は基礎控除（令和7年以降58〜95万円）超で申告義務\n" +
    "- ×「消費税は売上1000万円超で自動的に課税」= 基準期間（前々年）の課税売上で判定。当年に超えても翌々年から課税事業者\n" +
    "- ×「年末調整で全て終わる」= 医療費控除・寄付金控除・住宅ローン控除初年度などは確定申告必須\n" +
    "- ×「○月決算にすれば消費税の免税期間が延びる」= 免税期間は設立日と事業年度末の間隔で決まる。決算月自体に固有の優位性はない。どの月でも設立タイミングとの組み合わせで同じ効果が得られる\n" +
    "- ×根拠のない月・日・金額の断定 = 予定納税通知は7月中旬、消費税中間申告の期日は事業者区分で異なる、など事実に基づく\n" +
    "- ×特定の制度メリットが「○月決算だから」生じるかのような因果関係 = 決算月固有でない一般論を特定月の利点として語らない\n" +
    "topic の因果関係・時期・金額に少しでも疑問があれば、その topic は採用せず別の切り口に変える。「知って得する事実」ではなく「事実に反する煽り」を絶対に立案しないこと。";
